...
parser SwitchIngressParser(packet_in pkt,
    ...
    state parse_dns_query1 {
        ...
        transition select(p.label1.label) {
            ...
            default: parse_query1_over50;
        }
    }

    state parse_query1_over50 {
        counter2.set(p.label1.label);
        pkt.advance(400);
        counter2.decrement(8w50);
        transition select(counter2.is_zero()) {
            true: parse_dns_query2;
            false: parse_dns_query1_pre_repeat;
        }
    }

    state parse_dns_query1_pre_repeat {
        transition select(counter2.is_negative()) {
            true: parse_dns_query2;
            false: parse_dns_query1_repeat;
        }
    }

    state parse_dns_query1_repeat {
        pkt.advance(8);
        counter2.decrement(8w1);
        transition select(counter2.is_zero()) {
            true: parse_dns_query2;
            false: parse_dns_query1_repeat;
        }
    ...
    state parse_dns_query2 {
        ...
        transition select(p.label2.label) {
            ...
            32: parse_dns_q2_len32;
            33: parse_dns_q2_len33;
            34: parse_dns_q2_len34;
            35: parse_dns_q2_len35;
            default: parse_query2_over50;
        }
    }

    state parse_query2_over50 {
        counter2.set(p.label2.label);
        pkt.advance(400);
        counter2.decrement(8w50);
        transition select(counter2.is_zero()) {
            true: parse_dns_query3;
            false: parse_dns_query2_pre_repeat;
        }
    }

    state parse_dns_query2_pre_repeat {
        transition select(counter2.is_negative()) {
            true: parse_dns_query3;
            false: parse_dns_query2_repeat;
        }
    }

    state parse_dns_query2_repeat {
        pkt.advance(8);
        counter2.decrement(8w1);
        transition select(counter2.is_zero()) {
            true: parse_dns_query3;
    ...
    state parse_dns_q2_len32 {
        pkt.advance(256);
        transition parse_dns_query4;
    }
    state parse_dns_q2_len33 {
        pkt.advance(264);
        transition parse_dns_query4;
    }
    state parse_dns_q2_len34 {
        pkt.advance(272);
        transition parse_dns_query4;
    }
    state parse_dns_q2_len35 {
        pkt.advance(280);
        transition parse_dns_query4;
    }
    ...
    state parse_dns_query3 {
        ...
        transition select(p.label3.label) {
            ...
            default: parse_query3_over50;
        }
    }

    state parse_query3_over50 {
        counter2.set(p.label3.label);
        pkt.advance(400);
        counter2.decrement(8w50);
        transition select(counter2.is_zero()) {
            true: parse_dns_query4;
            false: parse_dns_query3_pre_repeat;
        }
    }

    state parse_dns_query3_pre_repeat {
        transition select(counter2.is_negative()) {
            true: parse_dns_query4;
            false: parse_dns_query3_repeat;
        }
    }

    state parse_dns_query3_repeat {
        pkt.advance(8);
        counter2.decrement(8w1);
        transition select(counter2.is_zero()) {
            true: parse_dns_query4;
    ...
    state parse_dns_query4 {
        ...
        transition select(p.label4.label) {
            ...
            default: parse_query4_over50;
        }
    }

    state parse_query4_over50 {
        counter2.set(p.label4.label);
        pkt.advance(400);
        counter2.decrement(8w50);
        transition select(counter2.is_zero()) {
            true: parse_dns_query5;
            false: parse_dns_query4_pre_repeat;
        }
    }

    state parse_dns_query4_pre_repeat {
        transition select(counter2.is_negative()) {
            true: parse_dns_query5;
            false: parse_dns_query4_repeat;
        }
    }

    state parse_dns_query4_repeat {
        pkt.advance(8);
        counter2.decrement(8w1);
        transition select(counter2.is_zero()) {
            true: parse_dns_query5;
    ...
    state parse_dns_query5 {
        ...
        transition select(p.label5.label) {
            ...
            default: parse_query5_over50;
        }
    }

    state parse_query5_over50 {
        counter2.set(p.label5.label);
//        pkt.advance(400);
//        counter2.decrement(8w50);
        pkt.advance(360);
        counter2.decrement(8w45);
        transition select(counter2.is_zero()) {
            true: parse_dns_query6;
            false: parse_dns_query5_pre_repeat;
        }
    }

    state parse_dns_query5_pre_repeat {
        transition select(counter2.is_negative()) {
            true: parse_dns_query6;
            false: parse_dns_query5_repeat;
        }
    }

    state parse_dns_query5_repeat {
        pkt.advance(8);
        counter2.decrement(8w1);
        transition select(counter2.is_zero()) {
            true: parse_dns_query6;
