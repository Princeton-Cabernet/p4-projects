...
*/
    ...
    apply {
        ...
        if(ig_md.parsed_answer == 1) {
            ...
            headers.netassay_hdr.used = 0;
            headers.netassay_hdr.domain_id = 0;
        }
        // HANDLE NORMAL, NON-DNS PACKETS
            ...
            if (sip_cip_matched == 1) {
                ...
                headers.netassay_hdr.domain_id = domain_reg_1_get_domain_action.execute(index_1);
                tstamp_reg_1_update_tstamp_action.execute(index_1);
            ...
            if (entry_matched == 0) {
                ...
                if (sip_cip_matched == 1) {
                    ...
                    headers.netassay_hdr.setValid();
                    headers.netassay_hdr.used = 1;
                    headers.netassay_hdr.domain_id = domain_reg_2_get_domain_action.execute(index_2);
                    tstamp_reg_2_update_tstamp_action.execute(index_2);
...
control SwitchEgress(
    ...
    RegisterAction<bit<32>,_,bit<1>> (byte_counts_table) byte_counts_table_reg_inc_action = {
        void apply(inout bit<32> value, out bit<1> overflow) {
            ...
            if (value >= 0x80000000) {
                ...
                overflow = 0;
                value = value + hdr.netassay_hdr.pkt_len;
            }
            overflow = 0;
        }
    };
    ...
    apply {
        if  (hdr.netassay_hdr.used == 1) {
            bit<16> domain_id = (bit<16>)hdr.netassay_hdr.domain_id;
            hdr.netassay_hdr.pkt_len = (bit<32>)(eg_intr_md.pkt_length - 4); // subtract 4 bytes, which is for frame check sequence (FCS).

            packet_counts_table_reg_inc_action.execute(domain_id);

            bit<1> overflow = byte_counts_table_reg_inc_action.execute(domain_id);

            if (overflow == 1) {
                byte_counts_table_overflow_reg_inc_action.execute(domain_id);
            }
        }
    }
}
